...
<div class="sk-cart-view" data-com="SkCartView">
  <template v-if="!nLock && items.length">
    <div class="s-cells s-top" @touchmove.prevent.stop>
      ...
      <div class="s-top_bar">
        <yu-button type="text" class="s-all" @click="fnAllCheck">
          <svg-icon v-if="nCheck && !hasUnCheck" class="s-check_box i-on" name="cart-check-on" slot="icon"></svg-icon>
          <svg-icon v-if="nCheck && hasUnCheck" class="s-check_box" name="cart-check" slot="icon"></svg-icon>
          <svg-icon v-if="!nCheck && hasUnCheck" class="s-check_box" name="cart-check" slot="icon"></svg-icon>
          <span>全选</span>
        </yu-button>
        <yu-button type="text" class="s-del" @click="fnDelCheck">删除</yu-button>
        <yu-button type="text" class="s-clear" @click="fnClear">清除无效商品</yu-button>
      </div>
    </div>
    <div class="s-cont">
      <!-- 活动商品 -->
      <template v-for="oDis in discounts">
        <div class="s-dis_items s-cells" v-if="oDis.range_type === 'partial'" :key="oDis.id">
          <div class="s-dis_info">
            ...
            <yu-button class="s-dis_tips" type="text" @click="fnDisDesc(oDis)">
              <svg-icon class="s-dis_ico" name="cart-explain" slot="icon"></svg-icon>
              <span>规则</span>
            </yu-button>
          ...
          <div class="s-dis_sum"><span v-if="oDis.discount_amount"><span>已优惠: </span><yu-currency :val="oDis.discount_amount"></yu-currency></span><span class="s-dis_sum_sub">小计: </span><yu-currency :val="oDis.match_item_amount - oDis.discount_amount"></yu-currency></div>
        </div>
      </template>
      <!-- 正常商品 -->
            ...
            <template v-for="oDis in discounts">
              <template v-if="oDis.range_type !== 'partial'">
                <tr v-if="oDis.discount_type === 'coupon'" :key="oDis.id">
                  <th>{{ oDis.name }}</th>
                  <td><span>可获得: </span><svg-icon class="s-coupon_ico" name="cart-coupon"></svg-icon></td>
                </tr>
                <tr v-else :key="oDis.id">
                  <th>{{ oDis.name }}</th>
                  <td><span>已优惠: </span><yu-currency :val="oDis.discount_amount"></yu-currency></td>
                </tr>
              </template>
            </template>
          </tbody>
    ...
    <div class="s-bar" @touchmove.prevent.stop>
      <div class="s-bar_info">
        <div class="s-sum"><span>合计: </span><yu-currency class="s-bar_price" :val="nSum"></yu-currency></div>
        <div v-if="nDisSum" class="s-tips">（不含运费，已优惠：<yu-currency :val="nDisSum"></yu-currency>）</div>
        <div v-else class="s-tips">（不含运费）</div>
      </div>
      <yu-button class="s-submit" type="danger" @click="fnSubmit">确认结算</yu-button>
    </div>
  </template>
  <template v-else>
    <div class="s-cells s-top" @touchmove.prevent.stop>
    ...
    <YuLoadTips class="s-load_tips" :is-loading="!!nLock" :is-empty="!nLock && !err && !items.length" :is-error="!nLock && !!err" empty-text="购物车里还没有商品" @retry="init">
      <svg-icon class="s-empty_icon" name="cart-empty" slot="empty_icon"></svg-icon>
      <yu-button class="s-go_buy" type="primary" @click="fnGoBuy" slot="empty_ext">去逛逛</yu-button>
    </YuLoadTips>
  </template>
</div>
</template>
      ...
      this.$sdk.cart.get(data => {
        ...
        if (data && data.res) {
          if (data.res.code === 200) {
            const _oCart = data.res.cart || {}

            this.items = _oCart.items || []

            this.$store.dispatch('cart/setCartCount', (_oCart.item_count || 0))
          } else {
...
.sk-cart-view {
  ...
  overflow: hidden;
  margin: 0 auto;
  padding: calc(44px + var(--statusBar) + var(--navBar)) 0 calc(var(--tabBar) + var(--homeIndicator) + 15px);
...
.s-bar {
  ...
  position: fixed;
  left: 0;
  bottom: calc(var(--tabBar) + var(--homeIndicator));
  display: flex;
  padding: 10px 15px;
  width: 100%;
  background-color: #fff;
