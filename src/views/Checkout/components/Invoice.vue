...
<template>
  <div v-if="allow_invoice_for_order" class="sk-checkout-invoice">
    <YuCell label="发票信息" class="s-checkbox" :class="fnCheckForm() && oInvoice.need_invoice ? 'is-checked' : ''" @click="fnChoose" :is-link="true">
      <span class="s-checkbox__label">
        {{oInvoiceForm.need_invoice ? getInvocieType(oInvoice.invoice_type) : '我要开发票'}}
      </span>
    </YuCell>
    ...
    <!-- <div class="s-info" v-if="fnCheckForm() && oInvoice.need_invoice">
      ...
      <a class="s-info-btn" href="javascript:void(0);" @click="fnEdit">修改</a>
    </div> -->

    <YuPopup v-model="showEdit" @close="fnClose">
      <div class="s-dialog-con">
        <div class="s-dialog-title">发票信息</div>
        ...
        <YuForm class="s-input-field" :model="oInvoiceForm" :rules="rules" ref="invoice" :label-position="isMobi ? 'top' : ''" label-width="123px">
          <div class="item">
            <h3 class="label">选择发票类型：</h3>
            <div>
          ...
          <div class="item">
            <!-- <h3 class="label">填写发票抬头：</h3> -->
            <YuField label="填写发票抬头：" placeholder="必填" type="text" v-model.trim="oInvoiceForm.invoice_title" required></YuField>
          </div>
          <div v-if="/(2|3)/i.test(oInvoiceForm.invoice_type)" prop="tax_number">
            <!-- <h3 class="label" data-label="请输入企业纳税人识别号："></h3> -->
            <YuField label="企业税号：" placeholder="请输入企业纳税人识别号" type="text" v-model.trim="oInvoiceForm.tax_number" required></YuField>
          </div>
          <template v-if="oInvoiceForm.invoice_type === 3">
            <div prop="company_bankname">
              <!-- <h3 class="label">开户银行：</h3> -->
              <YuField label="开户银行：" placeholder="请输入企业开户银行" type="text" v-model.trim="oInvoiceForm.company_bankname" required></YuField>
            </div>
            <div prop="company_bankcard">
              <!-- <h3 class="label">银行账号：</h3> -->
              <YuField label="银行账号：" placeholder="请输入企业银行账号" type="text" v-model.trim="oInvoiceForm.company_bankcard" required></YuField>
            </div>
            <div prop="company_address">
              <!-- <h3 class="label">企业地址：</h3> -->
              <YuField label="企业地址：" placeholder="请输入企业注册地址" type="text" v-model.trim="oInvoiceForm.company_address" required></YuField>
            </div>
            <div prop="company_phone_number">
              <!-- <h3 class="label">企业电话：</h3> -->
              <YuField label="企业电话：  " placeholder="请输入企业注册电话" type="text" v-model.trim="oInvoiceForm.company_phone_number" required></YuField>
            </div>
          </template>
          <div class="item">
            <h3 class="label">发票内容：</h3>
            <div>
            ...
            <div class="s-info-tips">
              <svg-icon name="info_tips" />
              {{oInvoiceForm.invoice_content === 0 ? '发票内容将显示详细商品名称与价格信息' : '发票内容将显示本单商品所属类别（如电器电子产品及配件）及价格信息'}}
            </div>
          </div>
          <div v-if="oInvoiceForm.invoice_style === 0">
            <!-- <h3 class="label">收票人手机：</h3> -->
            <YuField label="收票人手机：" placeholder="必填" type="text" v-model.number.trim="oInvoiceForm.invoice_mobile" required></YuField>
          </div>
          <div v-if="oInvoiceForm.invoice_style === 0" prop="invoice_email">
            <!-- <h3 class="label">收票人邮箱：</h3> -->
            <YuField label="收票人邮箱：" placeholder="非必填" type="text" v-model.trim="oInvoiceForm.invoice_email"></YuField>
          </div>
           <div v-if="oInvoiceForm.invoice_style === 1" prop="invoice_remark">
            <!-- <h3 class="label">备注：</h3> -->
            <div class="s-textarea-cont">
              <YuField
                label="请输入备注："
                placeholder="长度为100个字符"
                type="textarea"
                class="s-textarea"
                v-model="oInvoiceForm.invoice_remark" @input="fnRemarkInput" show-word-limit>
              </YuField>
        ...
        <div class="s-ctrl-btns">
          <YuButton type="primary" @click="fnSubmit">确定</YuButton>
          <YuButton type="button" @click="fnClose">取消</YuButton>
        </div>
      </div>
    </YuPopup>
  </div>
...
<style scoped>
  ...
  .sk-checkout-invoice /deep/ .yu-cell-hr {
...
.sk-checkout-invoice .yu-form {
  border: none;
}
.sk-checkout-invoice .yu-form .item {

}
.sk-checkout-invoice .yu-form .label {
  font-size: 14px;
  font-weight: 500;
  line-height: 44px;
  margin: 0;
}
.sk-checkout-invoice /deep/ .yu-field .yu-field-inner .yu-field-label {
  padding-left: 0 !important;
}
.sk-checkout-invoice /deep/ .yu-field .yu-field-input,
.sk-checkout-invoice /deep/ .yu-field .yu-field-label_text {
  line-height: 44px !important;
  height: auto;
}
.sk-checkout-invoice .s-dialog-title {
  font-size: 18px;
  color: #121212;
  text-align: center;
}
.sk-checkout-invoice /deep/ .yu-popup .s-dialog-con {
  margin: 0 !important;
}
</style>
